...
MobilePage.prototype.render = function ()
{
 ...
 * 新的论坛页面布局从这里开始
 */
NewThread.prototype.start = function ()
{
    this.makeSkeleton()
    this.deleteMultiplePageButtons()
    this.clearTags()
 ...
 * 去掉重复的文章tag
 */
NewThread.prototype.clearTags = function ()
{
    ...
    var tags = tagsNode.querySelectorAll('a') // 保存各项标签内容
    if (tags.length <= 1)
    {
        return
    }
 ...
 */
NewThread.prototype.deleteMultiplePageButtons = function ()
{
 ...
 */
NewThread.prototype.makeSkeleton = function ()
{
    ...
    this.makeAuthorInfo(sd)
    this.makeArticle(mn)
    this.makeComments(mn)
    this.makeOperationFunctions()
    this.makeRelatedThreads(sd)
    this.bind()
}
 ...
 * 仿制文章页面的样式来做帖子作者信息
 */
NewThread.prototype.makeAuthorInfo = function (elem)
{
    // 仿制文章页面的样式来做帖子作者信息
    // 会涉及到很多DOM操作，代码好难看啊
 ...
 * 仿制文章页面的样式来做帖子作者信息
 */
NewThread.prototype.makeArticle = function (elem)
{
    // 仿制文章页面的样式来做帖子作者信息
    // 会涉及到很多DOM操作，代码好难看啊
    var bgw = document.createElement('div')
 ...
 */
NewThread.prototype.makeComments = function (elem, source)
{
 ...
 */
NewThread.prototype.makeOperationFunctions = function ()
{
...
/**
 * 每一次更新页面都要重新添加操作按钮，不过这也是没办法的
 */
NewThread.prototype.makeOperationFunctionsAgain = function ()
{
    // 操作按钮原型
 ...
 */
NewThread.prototype.makeRelatedThreads = function (elem)
{
 ...
 */
NewThread.prototype.tool = function (elem)
{
 ...
 * 页面滚动的右侧“悬浮”效果
 */
NewThread.prototype.bindWindowScroll = function ()
{
 ...
 * 事件绑定函数
 */
NewThread.prototype.bind = function ()
{
    // 底部的页码按钮的点击事件
    var next = document.querySelector('.pgbtn a')
    var maxPageNumber = this.getMaxPageNumber()
    var self = this // remain `this`
    if (next)
    {
        next.onclick = function ()
        {
            var nextPageNumber = self.getNextPageNumber(this.getAttribute('href'))
            this.innerText = '请稍等'
            if (nextPageNumber <= maxPageNumber)
            {
                Z.ajax(
                {
                    ...
                    success: function (data)
                    {
                        ...
                        self.makeComments(document.querySelector('#skeleton .mn #comment'), source)
                        self.makeOperationFunctionsAgain()
                        ...
                        {
                            next.setAttribute('href', self.makeNewHref(next.getAttribute('href'), nextPageNumber))
                        }
                    },
 ...
 * 获得这一个帖子总共有多少页的回帖
 */
NewThread.prototype.getMaxPageNumber = function ()
{
    try
    {
 ...
 * 通过解析url得到下一页的标识数字
 */
NewThread.prototype.getNextPageNumber = function (href)
{
 ...
 * 根据提供的url生成下一个页面的url
 */
NewThread.prototype.makeNewHref = function (href, num)
{
    // 目前设置为获取五位数之内的回帖，超过这个数了这个文件应该已经不会使用了
 ...
 * 直播从这里开始
 */
LiveStart.prototype.start = function ()
{
    this.init()
    var cover = document.querySelector('#loadingCover')
    if (cover)
 ...
 * 加入内嵌样式，简化代码
 */
LiveStart.prototype.init = function ()
{
 ...
 * 初始化内容框架
 */
LiveStart.prototype.initSkeleton = function ()
{
    // 顶部banner
 ...
 * 这个部分没有发现bug
 */
LiveStart.prototype.initBanner = function (elem)
{
    var content = document.createElement('div')
 ...
 * 初始化侧边栏
 */
LiveStart.prototype.initSidebar = function (elem)
{
    var host = this.initSidebarHost()
    elem.appendChild(host)
    elem.appendChild(this.initSidebarShare())
    elem.appendChild(this.initSidebarRelate())
}

/**
 * 侧边栏直播人员块
 */
LiveStart.prototype.initSidebarHost = function ()
{
    ...
    p1.innerText = 'ZAEKE知客'

    var p2 = document.createElement('p')
    p2.classList.add('introInfo')
    p2.innerText = 'ZAEKE知客官方微博'

    var p3 = document.createElement('p')
    var a = document.createElement('a')
    a.setAttribute('href', 'http://weibo.com/zhikeshuma')
 ...
 * 侧边栏分享块
 */
LiveStart.prototype.initSidebarShare = function ()
{
    ...
    img2.setAttribute('alt', 'ZAEKE知客微信公众号')
    img2.setAttribute('id', 'wechatIcon')
    var p2 = document.createElement('p')
    p2.innerText = '关注“ZAEKE知客”'
    a2.appendChild(img2)
    a2.appendChild(p2)
 ...
 * 侧边栏相关文章（老板需求）
 */
LiveStart.prototype.initSidebarRelate = function ()
{
 ...
 * --'content'表示该项的文字信息
 */
LiveStart.prototype.allData = function ()
{
    var data = {}
...
/**
 * 根据obj提供的信息进行不同的渲染，得到不同的样式
 */
LiveStart.prototype.render = function (obj)
{
    var temp = ''
    if (obj.hasOwnProperty('single')) 
    {
        temp = this.renderSingle(obj)
    } 
    else if (obj.hasOwnProperty('video')) 
    {
        temp = this.renderVideo(obj)
    } 
    else 
    {
        temp = this.renderPhotos(obj)
    }
    return temp
}
 ...
 * 当直播内容只有一句话的时候该这么做
 */
LiveStart.prototype.renderSingle = function (obj)
{
...
/**
 * 当直播内容是一段视频（大部分是flash视频）的时候该这么做
 */
LiveStart.prototype.renderVideo = function (obj)
{
 ...
 * 当直播内容里面有图片的时候该这么做
 */
LiveStart.prototype.renderPhotos = function (obj)
{
    var photosTemp = ''
    var img // 迭代变量
 ...
 * 初始化内容区域
 */
LiveStart.prototype.initContent = function (elem)
{
    var allData = this.allData()
    for (var data in allData) 
    {
        var temp = this.render(allData[data])
        elem.innerHTML += temp
    }
}
 ...
 * 绑定各项事件处理内容
 */
LiveStart.prototype.initBind = function ()
{
    this.bindWechatIcon()
    this.bindContentPhotos()
    this.bindScrollLivePage()
    this.fix()
}
 ...
 * 这里没有发现bug
 */
LiveStart.prototype.bindWechatIcon = function ()
{
 ...
 * 经过hack之后，没有发现bug
 */
LiveStart.prototype.bindContentPhotos = function ()
{
 ...
 */
LiveStart.prototype.scale = function (elem, index)
{
    if (document.querySelector('#img_scale'))
    {
        return
    }
    ...
    // 代码写的有问题，会多绘制一个大图片出来……所以就提前检查一下
    if (parent.getAttribute('data-img-scale') === 'true')
    {
        return
    }
 ...
 * 页面的滚动事件
 */
LiveStart.prototype.bindScrollLivePage = function ()
{
 ...
 */
LiveStart.prototype.fix = function ()
{
...
/**
 * 辅助判断函数，判断显示total张图片需要多少行
 */
LiveStart.prototype.judgeLine = function (total, perLine)
{
    if (total % perLine === 0) 
    {
 ...
 * 输入内容时页面不刷新
 */
LiveStart.prototype.timer = function ()
{
...
/**
 * 如果不是官方团队则删除掉回复功能
 * 添加一个只有管理团队才能看到的按钮，用于后期的改动
 */
LiveStart.prototype.liveAuthor = function ()
{
 ...
 * 得到直播内容的总页数
 */
LiveStart.prototype.getMaxPageNumber = function ()
{
    try
    {
 ...
 * 通过解析url得到下一页的标识数字
 */
LiveStart.prototype.getNextPageNumber = function (href)
{
    return Number(href.split(/\-/g)[2])
}
 ...
 * 根据提供的url生成下一个页面的url
 */
LiveStart.prototype.makeNewHref = function (href, num)
{
    href = href.replace(/\-\d{1,5}\-/, '-' + num + '-');
    return href;
}
 ...
 * 根据返回得到的页面解析内容并返回
 */
LiveStart.prototype.data = function (source)
{
 ...
 * 拿到页面之后解析DOM，更新页面内容
 */
LiveStart.prototype.updateContent = function (source)
{
    var datas = this.data(source)
    var content = jQuery('#event .content').first();
    var _content = ''
    for (var data in datas) 
    {
        var temp = this.render(datas[data]);
        _content += temp
    }
 ...
 */
LiveStart.prototype.setAjaxEvent = function ()
{
    var next = document.querySelector('.pgbtn a');
    var maxPageNumber = this.getMaxPageNumber();
    if (next)

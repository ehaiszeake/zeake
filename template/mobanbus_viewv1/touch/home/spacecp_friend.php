...
<div class="bus_w100 bus_fl">
  <div class="bus_listbox bus_friend">
		<!--{if !$_G[inajax] && ($op == 'syn' || $op == 'find' || $op == 'search' || $op == 'group' || $op == 'request')}-->
			<h1 class="mt"><img alt="friend" src="{STATICURL}image/feed/friend.gif" class="vm" />
			<!--{if $op == 'find'}-->
		...
		<!--{if $op =='ignore'}-->
			...
			<form method="post" autocomplete="off" id="friendform_{$uid}" name="friendform_{$uid}" action="home.php?mod=spacecp&ac=friend&op=ignore&uid=$uid&confirm=1" {if $_G[inajax]}onsubmit="ajaxpost(this.id, 'return_$_GET[handlekey]');"{/if}>
				...
				<p class="o pns">
					<button type="submit" name="friendsubmit_btn" class="bus_btn bus_post_gray" value="true"><strong>{lang determine}</strong></button>
					<a class="bus_btn bus_post_gray" href="javascript:history.go(-1);"><I class="head_ico icon-angle-left"></I></a>
				</p>
			</form>
		...
		<!--{elseif $op=='add'}-->
			...
			<form method="post" autocomplete="off" id="addform_{$tospace[uid]}" name="addform_{$tospace[uid]}" action="home.php?mod=spacecp&ac=friend&op=add&uid=$tospace[uid]" {if $_G[inajax]}onsubmit="ajaxpost(this.id, 'return_$_GET[handlekey]');"{/if}>
				...
				<!--{if $_G[inajax]}--><input type="hidden" name="handlekey" value="$_GET[handlekey]" /><!--{/if}-->
				<input type="hidden" name="formhash" value="{FORMHASH}" />
				<div class="c bus_friend_adss">
					<table>
						<tr>
							<td valign="top" width="60" class="avt"><a href="home.php?mod=space&uid=$tospace[uid]"><!--{avatar($tospace[uid],small)}--></td>
						</tr>
						<tr>
				...
				<p class="o pns">
					<button type="submit" name="addsubmit_btn" id="addsubmit_btn" value="true" class="bus_btn bus_post_gray"><strong>{lang determine}</strong></button>
				</p>
			</form>
		<!--{elseif $op=='add2'}-->

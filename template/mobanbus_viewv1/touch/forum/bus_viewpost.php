...
<div class="bus_rpos">
	...
	<div class="mobanbus_bd">
		...
		<!--{/if}-->
		<li class="bus_fl"><a href="forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]&reppost=$_G[forum_firstpid]&page=$page" class="y" style="height:30px;width:30px;margin-top:7px;background:url({STATICURL}image/mobile/images/icon_photo.png) no-repeat"><span class="none">{lang publish}</span></a><!--{hook/viewthread_fastpost_button_mobile}--></li>
		
		<li><input type="text" value="{lang send_reply_fast_tip}" class="input grey" color="gray" name="message" id="fastpostmessage"></li>
		<li class="bus_fr" id="fastpostsubmitline"><!--{if $secqaacheck || $seccodecheck}--><!--{subtemplate common/seccheck}--><!--{/if}-->
		<input type="button" value="{lang publish}" class="fastpost_btn bus_btn" name="replysubmit" id="fastpostsubmit"></li>
		</ul>
	</div>
	</div>
    </form>

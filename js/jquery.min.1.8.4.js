//适配 REM（移动端）
(function(doc){var d=document.documentElement,t=null,w,l;function changeRem(){w=d.getBoundingClientRect().width;if(l===w){return;}
l=w;doc.rem=(w/375)*100;if(/ZTE U930_TD/.test(navigator.userAgent)){doc.rem=doc.rem*1.13;}
d.style.fontSize=doc.rem+'px';}
changeRem();doc.addEventListener('resize',function(){if(document.activeElement.tagName!="INPUT" || document.activeElement.tagName!="TEXTAREA"){clearTimeout(t);t=setTimeout(changeRem,00);}});doc.addEventListener('orientationchange',function(){clearTimeout(t);t=setTimeout(changeRem,300);});})(window,undefined);	
...
/* Lazy Load - jQuery plugin for lazy loading images	Version:  1.9.3 */
(function(c,e,h,k){var f=c(e);c.fn.lazyload=function(a){function b(){var a=0;g.each(function(){var b=c(this);if(!(d.skip_invisible&&!b.is(":visible")||c.abovethetop(this,d)||c.leftofbegin(this,d)))if(!c.belowthefold(this,d)&&!c.rightoffold(this,d))b.trigger("appear"),a=0;else if(++a>d.failure_limit)return!1})}var g=this,d={threshold:0,failure_limit:0,event:"scroll",effect:"show",container:e,data_attribute:"src",skip_invisible:!0,appear:null,load:null,placeholder:"data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAADsQAAA7EAZUrDhsAAAANSURBVBhXYzh8+PB/AAffA0nNPuCLAAAAAElFTkSuQmCC"};
...
var units={
		//units.regular.m.test("hello")==true ，说明是手机号码
		regular:{
			"account":/^[a-zA-Z]{1}([a-zA-Z0-9]|[._]){4,19}$/,					/* 5-20个以字母开头、可带数字、"_"、"."的字串  */
			"psword":/^(\w){6,20}$/,											/* 只能输入6-20个字母、数字、下划线 */
			"txt":/[\w\W]+/,													/*不能为空！*/
			"txt6_16":/^[\w\W]{6,16}$/,											/*请填写6到16位任意字符！*/
			"txt3_20":/^[\w\W]{3,20}$/,											/*请填写3到20位任意字符！*/
			"n":/^\d+$/,														/*请填写数字！*/
			"n6_16":/^\d{6,16}$/,												/*请填写6到16位数字！*/
			"s":/^[\u4E00-\u9FA5\uf900-\ufa2d\w\.\s]+$/,						/*不能输入特殊字符！*/
			"s6_18":/^[\u4E00-\u9FA5\uf900-\ufa2d\w\.\s]{6,18}$/,				/*请填写6到18位字符！*/
			"p":/^[0-9]{6}$/,													/*请填写邮政编码！*/
			"m":/^13[0-9]{9}$|14[0-9]{9}|15[0-9]{9}$|17[0-9]{9}$|18[0-9]{9}$/,	/*请填写手机号码！*/
			"e":/^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/,				/*邮箱地址格式！*/
			"url":/^(\w+:\/\/)?\w+(\.\w+)+.*$/,									/*请填写网址！*/
			"idNum":/^(\d{15}$|^\d{18}$|^\d{17}(\d|X|x))$/						/*身份证号码*/
		},		/* 
		
			* 距离现在的时间 参数为时间戳
			* units.getDateDiff(new Date().getTime());
			
		*/
		getDateDiff:function(TimeStamp){
			var minute = 1000 * 60,hour = minute * 60,day = hour * 24,halfamonth = day * 15,month = day * 30,year = month * 12,now = new Date().getTime(),diffValue = now - TimeStamp,
				yearC =diffValue/year,monthC =diffValue/month,weekC =diffValue/(7*day),dayC =diffValue/day,hourC =diffValue/hour,minC =diffValue/minute;
			if(yearC>=1){result=parseInt(yearC) + "年前";}
			else if(monthC>=1){result=parseInt(monthC) + "个月前";}
		...
		strLen:function(chars) {var sum = 0;for (var i=0;i<chars.length; i++){var c = chars.charCodeAt(i);if ((c >= 0x0001 && c <= 0x007e) || (0xff60<=c && c<=0xff9f)){sum++;} else {sum+=2;}} return sum;},
		sys:{
		...
		//图片预加载
		imgPreLoad:function(){
			var i=0,loaded=0,handle,imgList=arguments[0],len=imgList.length,fun1=arguments[1],fun2=arguments[2];
			for(;i<len;i++){
				(function (i){
					var oImg = new Image();
					oImg.src = imgList[i];
					if(oImg.complete){
						...
						if(per >= 1){if(fun2!==undefined){fun2();/*clearTimeout(handle);*/}}
					}else {
...
function loadImg(){
	$("img").each(function(){
		if($(this).attr("src").indexOf("http")<0){
			$(this).attr("src",golbalIp+"/"+$(this).attr("src"));
		}
	});
}
...
//	loadImg();
	...
	$("body").on("click submit input propertychange copy cut paste selectstart",function(e){
		...
		if(e.type=="submit" && t.hasClass("login_form")){
			//登录表单验证
			var ipt1=t.find(".login_username").val(),
				ipt2=t.find(".login_password").val();
			if($.trim(ipt1)==""){
				$(".username_password_tip").html("请填写用户名！");
			}else if($.trim(ipt2)==""){
				$(".username_password_tip").html("请填写登录密码！");
			}else {
				$(".username_password_tip").html("");
				//锁住ajax请求
				if(!variable_obj.login_form_lock){
					variable_obj.login_form_lock=true;
					$.ajax({
						url:golbalIp+"/user/login",
						type:"post",
						dataType:"json",
						data:{"account":ipt1,"password":ipt2},
						beforeSend:function(){},
						success:function(data){
							console.log(data);
							if(data.code==1){
								console.log($.cookie('bz'));
								$.removeCookie('bz',{ expires:365 });
								$(".success_tip").find(".success_tip_inner").html("登录成功").end().removeClass("f_dn");
								$(".no_login").addClass("f_dn").next(".logined").removeClass("f_dn").find(".name:eq(0)").html(data.data.account);
								setTimeout(function(){$(".success_tip").addClass("f_dn");window.location.reload(true);},1500);
								$.cookie('bz',JSON.stringify(data.data),{ expires:365 });
								$(".login_wrapper").find(".close_pop_wrapper").trigger("click");
							}else {
								$(".username_password_tip").html(data.message);
							}
							variable_obj.login_form_lock=false;
							
						},error:function(){
							$(".username_password_tip").html("网络错误，请刷新页面重新访问！");
							variable_obj.login_form_lock=false;
						}
					})
				}				
			...
			if(units.strLen(ipt1.val())<3 || units.strLen(ipt1.val())>15){
				...
				if(!variable_obj.register_form_lock){
					...
					$.ajax({
						...
						success:function(data){
							if(data.code==1){
								$(".success_tip").find(".success_tip_inner").html("注册成功，请登录！").end().removeClass("f_dn");
								setTimeout(function(){$(".success_tip").addClass("f_dn");$(".login_page").trigger("click");},2000);
							}else if(data.code==4){
								...
								$(".username_password_email_tip").html(data.message);
							}
							variable_obj.register_form_lock=false;
						},error:function(){
							$(".username_password_email_tip").html("网络错误，请刷新页面重新访问！");	
							variable_obj.register_form_lock=false;
						}
					})
				}				
			...
			return false;
		}else if(e.type=="submit" && t.hasClass("search_form")){
			//使用邮箱验证码找回密码
			var ipt1=t.find(".search_register_username").val(),
				ipt2=t.find(".search_register_password").val();
			if(!units.regular.e.test(ipt1)){
				$(".search_email_code_tip").html("请填写正确的邮箱格式！");
			}else if($.trim(ipt2)==""){
				$(".search_email_code_tip").html("请填写邮箱验证码！");
			}else {
				$(".search_email_code_tip").html("");
				//锁住ajax请求
				if(!variable_obj.search_form_lock){
					variable_obj.search_form_lock=true;
					...
					$.ajax({
						...
						success:function(data){
							...
							if(data.code==1){
								...
								setTimeout(function(){
									$(".search_wrapper").addClass("f_dn");
									$(".modify_psw_wrapper").removeClass("f_dn");
								},1500);
								
							}else {
								$(".search_email_code_tip").html(data.message);
							}
							variable_obj.search_form_lock=false;
							
						},error:function(){
							$(".search_email_code_tip").html("网络错误，请刷新页面重新访问！");
							variable_obj.search_form_lock=false;
						}
					})
			...
			return false;
		}else if(e.type=="submit" && t.hasClass("modify_psw_form")){
			//找回密码-修改密码
			var ipt1=t.find(".modify_new_psw").val(),
				ipt2=t.find(".modify_new_psw_sure").val();
			if(units.strLen(ipt1)<6){
				$(".modify_new_psw_tip").html("密码（不小于6个字符）");
			}else if(ipt1!=ipt2){
				$(".modify_new_psw_tip").html("密码不一致！");
			}else {
				$(".modify_new_psw_tip").html("");
				//锁住ajax请求
				if(!variable_obj.modify_psw_form_lock){
					variable_obj.modify_psw_form_lock=true;
					...
					$.ajax({
						...
						success:function(data){
							...
							if(data.code==1){
								...
								$(".success_tip").find(".success_tip_inner").html("密码修改成功，请登录!").end().removeClass("f_dn");
								setTimeout(function(){
									$(".success_tip").addClass("f_dn");
									$(".modify_psw_wrapper").addClass("f_dn");
									$(".modify_psw_wrapper").find(".login_page").trigger("click");
								},1500);
								
							}else {
								$(".modify_new_psw_tip").html(data.message);
							}
							variable_obj.modify_psw_form_lock=false;
							
						},error:function(){
							$(".modify_new_psw_tip").html("网络错误，请刷新页面重新访问！");
							variable_obj.modify_psw_form_lock=false;
						}
					})
			...
			if(t.hasClass("for_text")){
				t.next("input").focus();
			}else if(t.hasClass("weixin")){
				...
				$(".login_wrapper2").addClass("f_dn");
				$(".username_password_email_tip,.username_password_tip,.search_email_code_tip,.modify_new_psw_tip").html("");
			}else if(t.hasClass("close_pop_wrapper")){
				$(".pop_wrapper").fadeOut(function(){
					variable_obj.enabledScroll(); 
					t.parents(".pop_box").addClass("f_dn");
					$(this).find("form").each(function(){this.reset();$(this).find("label").removeAttr("style")});
					$(".username_password_email_tip,.username_password_tip,.search_email_code_tip,.modify_new_psw_tip").html("");
				});				
			...
			}else if(t.hasClass("js_logout")){
				$.removeCookie('bz',{ expires:365 });
				$(".no_login").removeClass("f_dn").next(".logined").addClass("f_dn").find(".name:eq(0)").html("");
				$(".success_tip").find(".success_tip_inner").html("退出成功！").end().removeClass("f_dn");
				setTimeout(function(){$(".success_tip").addClass("f_dn");},2000);
			}else if(t.hasClass("my_home_page_a")){
				window.location.href=t.attr("data-type");	
			}else if(t.hasClass("jump_personal_info")){
				//点击昵称跳转到个人中心信息
				if($.cookie('bz')!==undefined && $.trim($.cookie('bz'))!=="" && ((new Date().getTime()/1000-JSON.parse($.cookie('bz')).updated) < 3600)){
					//获取个人资料
					units.lockPerosonalInfo=false;
					if(!units.lockPerosonalInfo){
						units.lockPerosonalInfo=true;
						$.ajax({
							...
							error:function(){alert("网络错误，请稍后再试！");units.lockPerosonalInfo=false;}		
						});
					}
						
				}
			}
		}else if(e.type=="input" || e.type=="propertychange"){
			if(t.hasClass("placeholder")){
				...
				//搜索框判断
				if(t.attr("id")=="keyword"){
					if($.trim(t.val())!=""){
						$(".search_btn").addClass("active");
					}else {
						$(".search_btn").removeClass("active");
					}
				}else if(t.hasClass("comment_des")){
					if($.trim(t.val())!=""){
						t.next(".public_btn").addClass("active");
					}else {
						t.next(".public_btn").removeClass("active");
					}
				}
			}
		}else if(e.type=="copy" || e.type=="cut" || e.type=="paste" || e.type=="selectstart"){
			if(t.hasClass("js_noCopyCutPaste")){
				e.preventDefault();
				return false;
			}
		}
	}).append(variable_obj.html);
	...
	//移动端处理
	if(variable_obj.sys.ios() || variable_obj.sys.adr() || variable_obj.sys.wip()){
		$(window).off("scroll");
		$(document.body).on('touchstart', function(event) {variable_obj.touchPointer = event.originalEvent.changedTouches[0].clientY; }); 
		$(document.body).on('touchmove', function(event) {
			var y = event.originalEvent.changedTouches[0].clientY, 
				direction=variable_obj.touchPointer-y;
				if (direction>0) {
					variable_obj.touchPointer = y;  
					if(!$("body").hasClass("little")){$("body").addClass("little");}
				} else {
					if($("body").hasClass("little")){$("body").removeClass("little");}
				}
	...
	//移动端打开导航
	$(".mobile_nav").on("click touchstart",function(e){
		if(e.type=="touchstart"){e.preventDefault();}
		if(!$(this).hasClass("active")){
			$(".mobile_nav,.header").addClass("active");	
		}else {
			$(".mobile_nav,.header").removeClass("active");
		}
	});
...
//				//请求成功
//				var html="";
//				data.data.push({"id":0,"name":"知客专题","title":"知客专题"});
//				data.data.push({"id":-1,"name":"名人堂","title":"名人堂"});
//				
...
//			if(data.code==1){
//				//请求成功
//				var html='<dl class="f_fl"><dt>友情链接</dt>';
//				$.each(data.data.list,function(i,item){
//					html+='<dd class="f_toe"><a href="'+item.url+'" target="_blank">'+item.title+'</a></dd>';
//				});
//				$(".link_qrcode").find("dl:last").html(html+"</dl>");
//			}
//		},
//		error:function(){}		
//	})
	...
	if($.cookie('bz')===undefined || $.trim($.cookie('bz'))==="" || ((new Date().getTime()/1000-JSON.parse($.cookie('bz')).updated) > 3600)){
		...
		$.ajax({
			url:golbalIp+"/user/message_unread",
			type:"get",
			dataType:"json",
			data:{"account":JSON.parse($.cookie('bz')).account,"token":JSON.parse($.cookie('bz')).token},
			beforeSend:function(){},
			success:function(data){
				console.log(data);
				if(data.code==1){
					data.data.total==0 ? $(".message_num").hide() : $(".message_num").html(data.data.total);
				}
			},
			error:function(){}		
		});
			...
			success:function(data){
				...
				if(data.code==1){
					...
					if(data.data.type==0){
						...
						$(".pc_dropDown_inner").find("a:eq(2)").hide();
						$(".writing").hide();
						$(".pc_fun_tab").find("li:eq(3)").hide();
						$(".pc_dropDown_inner").find("a:eq(3)").hide();
					}else if(data.data.type==1){
						$(".pc_dropDown_inner").find("a:eq(3)").hide();
					}
						
					if($(".logined").length){
						$(".logined").find("img").attr("src",data.data.img);
					}
				}
			},
	...
	$(".check_code").on("click",function(){
		var val=$("#register_mobile").val();
			if(!units.regular.m.test(val)){
				alert("手机号码格式不正确");
			}else {
				if(!sendLock){
					sendLock=true;
					$.ajax({
						url:golbalIp+"/main/sendmsm",
						type:"get",
						dataType:"json",
						data:{"phone":val},
						beforeSend:function(){},
						...
						error:function(){alert("网络错误，请稍后再试！");sendLock=false;}		
					});
				}
			}
	});
